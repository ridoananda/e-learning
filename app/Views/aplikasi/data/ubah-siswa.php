          ...
          <div class="col-9">
            <div class="form-group">
              <select class="form-control" name="mapel_id" id="mapel_id">
                <?php $current_mapel =  $db->table('user_mapel')->where(['id' => $siswa['mapel_id']])->get()->getRowArray() ?>
                <option value="<?= $siswa['mapel_id']; ?>"><?= $current_mapel['kelas']; ?> - <?= $current_mapel['jurusan']; ?></option>
                <?php foreach ($user_mapel as $m) : ?>
                  <?php if ($m['kelas'] != $current_mapel['kelas']) : ?>
                    <option value="<?= $m['id']; ?>"><?= $m['kelas']; ?> - <?= $current_mapel['jurusan'] ?></option>
                  <?php endif ?>
                <?php endforeach ?>
              </select>
            </div>
